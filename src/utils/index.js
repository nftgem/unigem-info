...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `${process.env.REACT_APP_UNIGEM_URL}/#/` +
      (remove ? `remove` : `add`) +
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `${process.env.REACT_APP_UNIGEM_URL}/#/swap?inputCurrency=${token0Address}`
  } else {
    return `${process.env.REACT_APP_UNIGEM_URL}/#/swap?inputCurrency=${
